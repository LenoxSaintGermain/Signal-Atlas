...
| E15-S05 ElevenLabs Secondary Voice Lane Planning | E15 | P1 | In Progress | Cloud Run API env now stages ElevenLabs key/agent metadata, admin readiness visibility is live, the team can flip the public intake lane between Gemini and ElevenLabs, and the public intake mounts the ElevenLabs Chief of Staff widget when selected; deeper provider routing remains queued. |
...
| E15-S07 ElevenLabs Ghost Agent Lane | E15 | P1 | Queued | Replace the widget-only ElevenLabs intake lane with an SDK-backed Ghost agent using client tools, action feed telemetry, and a signed-session path as defined in `docs/mvp/elevenlabs_ghost_agent_lane_spec.md`. |
