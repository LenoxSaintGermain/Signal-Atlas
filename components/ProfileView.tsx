...
const SectionFrame = ({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) => (
  <section className="space-y-4 border border-black/10 bg-white p-6 shadow-[0_18px_50px_-44px_rgba(40,33,30,0.35)] md:p-8">
...
const FindingList = ({ items }: { items: DnaFinding[] }) => (
  <div className="grid gap-4">
    {items.map((item, index) => (
      <div key={`${item.label}-${index}`} className="border border-black/10 bg-[#FBF8F2] p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="text-xl text-[#09161a] font-editorial">{item.label}</div>
          <div className="border border-black/10 px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-black/45 font-data">
            {item.evidence}
          </div>
        </div>
...
const MarkerGrid = ({ markers }: { markers: DnaMarker[] }) => (
  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
    {markers.map((marker) => (
      <div key={marker.id} className="border border-black/10 bg-[#FBF8F2] p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="text-[10px] uppercase tracking-[0.22em] text-black/45 font-data">{marker.label}</div>
          <div className={`border px-3 py-1 text-[10px] uppercase tracking-[0.18em] ${SCORE_TONE[marker.band] || SCORE_TONE.viable}`}>
            {marker.band}
          </div>
        </div>
...
const CompensationCard = ({ value }: { value: CompensationPosture }) => (
  <div className="grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
    <div className="border border-[#28211E]/12 bg-[#28211E] p-6 text-[#f7f1e8] shadow-[0_26px_70px_-52px_rgba(40,33,30,0.85)]">
      <div className="text-[10px] uppercase tracking-[0.28em] text-[#8DD9BF] font-data">Market Value</div>
      <div className="mt-4 text-4xl leading-tight text-white font-editorial">{value.market_value_range}</div>
      <div className="mt-6 text-[10px] uppercase tracking-[0.22em] text-[#8DD9BF] font-data">Target Ask</div>
...
const EvidenceNotes = ({ notes }: { notes: DnaEvidenceNote[] }) => (
  <div className="grid gap-4 lg:grid-cols-3">
    {notes.map((note, index) => (
      <div key={`${note.source_label}-${index}`} className="border border-black/10 bg-[#FBF8F2] p-5">
        <div className="flex items-center justify-between gap-3">
...
export function ProfileView(props: { profile: ProfileContent }) {
  ...
  const sections: Record<string, React.ReactNode | null> = {
    ...
    extinction_risks: hasItems(profile.extinction_risks) ? (
      <div className="border border-red-500/15 bg-red-50/70 p-5">{renderBullets(profile.extinction_risks, 'red')}</div>
    ) : null,
    adaptive_assets: hasItems(profile.adaptive_assets) ? (
      <div className="border border-emerald-500/15 bg-emerald-50/70 p-5">{renderBullets(profile.adaptive_assets)}</div>
    ) : null,
    lean_into: hasItems(profile.lean_into) ? (
      <div className="border border-black/10 bg-[#FBF8F2] p-5">{renderBullets(profile.lean_into)}</div>
    ) : null,
    let_go: hasItems(profile.let_go) ? (
      <div className="border border-black/10 bg-[#FBF8F2] p-5">{renderBullets(profile.let_go, 'red')}</div>
    ) : null,
    build_next: hasItems(profile.build_next) ? (
      <div className="border border-black/10 bg-[#FBF8F2] p-5">{renderBullets(profile.build_next, 'amber')}</div>
    ) : null,
    evolution_path_90_days: hasItems(profile.evolution_path_90_days) ? (
      <div className="border border-black/10 bg-[#FBF8F2] p-5">{renderBullets(profile.evolution_path_90_days, 'ink')}</div>
    ) : null,
  };
    ...
    <div className="space-y-8 md:space-y-10">
      ...
      <section className="overflow-hidden border border-[#28211E]/12 bg-[#28211E] text-[#f7f1e8] shadow-[0_26px_70px_-52px_rgba(40,33,30,0.85)]">
        <div className="grid gap-8 px-6 py-7 md:px-8 md:py-8 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
          <div>
            <div className="text-[10px] uppercase tracking-[0.32em] text-[#8DD9BF] font-data">Adaptive Career Research Dossier</div>
          ...
          <div className="border border-white/10 bg-white/[0.04] p-5 md:p-6">
            <div className="text-[10px] uppercase tracking-[0.3em] text-[#8DD9BF] font-data">Target Environment</div>
